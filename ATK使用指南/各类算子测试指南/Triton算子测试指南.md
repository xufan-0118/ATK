  ...
  - name:
    ...
    shapes:
      dim_numbers:
        values: [ 1, 2, 3, 4, 5 ]
      dim_values: 
        values: [ 1,2,3,4,5,7,8,11,15,16,17,22,23,25,27,29,31,32,33,37,39,255,256,257 ]
      max_length: 196608
  - name:
    type: tensor
    required: true
...
关键参数设置：`-tup --triton_ut_path`，表示包含triton kernel 函数(triton_add)的文件所在目录。
例如包含`test_general_add.py`文件目录如下，则需要传入`-tup ./generalization_cases`
...
│   .....
```
一些现成的triton kernel, 可从triton-ascend仓下载：[generalization_cases](https://gitcode.com/Ascend/triton-ascend/blob/master/ascend/examples/generalization_cases)
