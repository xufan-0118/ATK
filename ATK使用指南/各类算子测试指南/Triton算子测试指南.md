...
关键参数设置：`-tup`，可选参数，表示包含待测 triton kernel 函数`triton_add`的文件所在目录。
例如，测试的`triton_add`接口在`test_general_add.py`文件中，包含该文件的目录为`test_kernel`，则需要传入参数`-tup ./test_kernel`。
...
```
atk node --backend triton --devices 0 node --backend npu --devices 0 task -c all_torch.add.json --task performance_device -tup ./test_kernel -p triton_add.py
```
...
# 例：
atk node --backend triton --devices 0 node --backend cpu --is_compare False node --backend triton --devices 0 --bm_file all_torch.add_reports_2025-05-27-19-47-45.xlsx task -c ./result/torch.add/json/all_torch.add.json --task performance_device -tup ./test_kernel -e 10 -p triton_add.py
```
